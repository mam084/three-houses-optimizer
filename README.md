...
- [x] **Split `src/optimizer.py` and `app.py` into smaller, single-concern modules** *(this release)* - `optimizer.py` is now the `src/optimizer/` package and `app.py` is a slim entry point plus `app_state.py` and a `tabs/` package, with zero intended behavior change (see Known Simplifications for verification)
