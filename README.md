...
- **Byleth and the route's own lord are mandatory in Team Builder** - both are force-deployed story units on their route in the actual game, so they're always included (and can't be excluded) once you pick a specific route, same as the game requires
...
- **Import a build from the Character Optimizer into the Team Builder** - once you've picked a character's class (including any mix-and-match override), "Import this build into Team Builder" locks that exact path/stats in for the Team Builder tab, so it's used as-is (and the character is force-included) instead of Team Builder silently recomputing its own recommendation for them.
- **Class Explorer** - a third tab for browsing any class's own stat-boost profile as a bar chart, with its tier and certification requirement, and an optional side-by-side comparison against a second class. (Growth rates belong to the *character* in this game, not the class - see Known Simplifications - so this shows the number that actually plays that role: each class's flat stat boost.)
...
- **"Weapon rank growth per class" is modeled as a proficiency-match bonus, not a weapon-exp simulation.** A class whose certification weapon overlaps a character's own starting weapon talent (`data/character_weapon_talent.csv`) gets a small score bonus (`weapon_growth_bonus` in `src/optimizer.py`) - "practicing in a class that uses a weapon you're already good at is how that rank climbs fastest" - but this is a nudge on top of the existing stat-boost scoring, not a simulation of weapon-exp gain over time (see the class-progression simplification above, which this doesn't change).
...
**`app.py`** - the Streamlit dashboard (Character Optimizer, Team Builder, and Class Explorer tabs).
...
- [x] **Import a Character Optimizer build into the Team Builder, and a Class Explorer tab for browsing class stat boosts** *(this release)*
...
Character and class data via [Serenes Forest](https://serenesforest.net/three-houses/), a fan-maintained Fire Emblem reference site. Base stats, growth rates, and class stat boosts are scraped directly (`src/scrape_serenes.py`); class eligibility, character gender, class weapon requirements, character weapon talent, recruitment requirements, and character starting level are hand-curated from the same site and other Three Houses recruitment guides (see Known Simplifications) since none of that data is in a cleanly scrapable tabular form.
